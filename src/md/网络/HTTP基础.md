...
- `Warning`：警告内容
- `content-type`：用于定义网络文件的类型和网页的编码，决定浏览器将以什么形式、什么编码读取这个文件，服务用这个标头告诉浏览器返回内容的类型

  - [`mime`类型](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Basics_of_HTTP/MIME_types)：用来表示文档、文件或字节流的性质和格式

  - `charset`字符集：字符编码标准，告知浏览器此页面属于什么字符编码格式，下一步浏览器做好“翻译”工作
    - `ASCII`：并没有涵盖西文字母之外的字符
    - `gb2312`：代表国家标准第2312条，兼容`ASCII`，包含了6000多汉字，但是其中是不包含繁体的
    - `gbk`：`gb2312`国家标准扩展版，增加了繁体，包含所有亚洲字符集
    - `unicode`：万国码，统一编码
    - `utf-8`：`unicode`统一了编码，但是没有规定如何储存，`utf-8`指定了比较节省空间的储存方式来编码

  - `boundary`：用于告知服务器如何分割表单数据
