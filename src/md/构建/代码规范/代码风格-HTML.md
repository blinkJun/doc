...
3. **强制** 元素 id 必须保证页面唯一
4. **建议** `inline`元素不可包含`block`元素
5. **建议** `class`代表相应模块或部件的内容或功能，不以样式信息进行命名
...
8. **建议** 为含有多个子元素或具有深层结构的标签进行换行和相对父级缩进，过长的代码不容易阅读与维护，每行不得超过 `120` 个字符。

9. **建议** 使用样式或者一个标签可以实现的布局不要嵌套太多标签，尽量简洁
